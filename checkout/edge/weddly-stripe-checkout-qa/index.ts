  ...
  if(req.method==='GET'){
    try{
      ...
      if(sessionId){
        ...
        if(!provision?.activationUrl||!provision?.activationCode)return json({ok:false,error:'license_provision_failed'},500);
        const edition=provision.edition==='signature'?'ONE Signature':'ONE Essential';
        const amount=provision.edition==='signature'?'49,90 €':'39,90 €';
        const html='<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><title>WeddlySmartDesign · Pago de prueba confirmado</title><style>body{margin:0;background:#2f2927;font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#4a3a36}.wrap{max-width:560px;min-height:100vh;margin:auto;background:#f7eee5;padding:32px 22px;box-sizing:border-box}.badge{display:inline-block;background:#fff;border:1px solid #daccc3;border-radius:999px;padding:7px 10px;font-size:12px;font-weight:800}h1{font-family:Georgia,serif;font-weight:500;font-size:34px;margin:24px 0 10px}.card{margin-top:22px;background:#fffaf5;border:1px solid #daccc3;border-radius:16px;padding:18px}.code{display:block;margin:14px 0;padding:13px;background:white;border:1px solid #daccc3;border-radius:10px;word-break:break-all;font-family:ui-monospace,monospace;font-size:13px}.btn{display:block;text-align:center;background:#4a3a36;color:white;text-decoration:none;padding:14px;border-radius:12px;font-weight:800;margin-top:14px}.note{font-size:13px;line-height:1.5;color:#6d5d58;margin-top:14px}</style></head><body><main class="wrap"><span class="badge">SANDBOX · PAGO FICTICIO</span><h1>Pago de prueba confirmado.</h1><p>'+edition+' · '+amount+'</p><div class="card"><b>La licencia se ha creado correctamente.</b><span class="code">'+provision.activationCode+'</span><a class="btn" href="'+provision.activationUrl+'">Abrir activación de ONE</a><p class="note"><b>Importante:</b> si este navegador ya tiene una boda activa, WeddlySmartDesign recuperará esa boda en lugar de sustituirla. Para probar esta nueva licencia, abre el enlace de activación en una pestaña de incógnito o en otro navegador.</p></div></main></body></html>';
