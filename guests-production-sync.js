(()=>{
  ...
  G.token='';G.ver=0;G.last='';G.pushing=false;G.remote=false;G.identity='';
  ...
  function mergeSharedIdentity(state){if(!state||typeof state!=='object')return state;const id=sharedIdentity();if(!id.p1||!id.p2)return state;state.meta=state.meta||{};const old=Array.isArray(state.meta.couple)&&state.meta.couple.length>=2?[String(state.meta.couple[0]||''),String(state.meta.couple[1]||'')]:['',''];if(old[0]&&old[1]&&(old[0]!==id.p1||old[1]!==id.p2)){Object.values(state.guests||{}).forEach(g=>{if(!g?.group)return;let v=String(g.group);v=v.replace(new RegExp(`^(Familia|Amigos) ${escReg(old[0])}$`,'i'),(_,k)=>`${k} ${id.p1}`);v=v.replace(new RegExp(`^(Familia|Amigos) ${escReg(old[1])}$`,'i'),(_,k)=>`${k} ${id.p2}`);g.group=v})}state.meta.couple=[id.p1,id.p2];state.meta.weddingDate=id.date||'';return state}
  ...
  async function start(){
    ...
    let local=mergeSharedIdentity(parse(read()));
    if(local)write(local);
    ...
    try{
      ...
      if(a.x.state){
        ...
        if(merged!==original){const p=await api('PUT',{version:G.ver,state});if(p.r.ok&&p.x.ok)G.ver=+p.x.version||G.ver+1;else if(p.r.status===409&&p.x.server?.state){const latest=mergeSharedIdentity(p.x.server.state),q=await api('PUT',{version:+p.x.server.version||G.ver,state:latest});if(q.r.ok&&q.x.ok){G.ver=+q.x.version||(+p.x.server.version||G.ver)+1;write(latest);G.last=JSON.stringify(latest);G.identity=identityOf(latest);open();return}}}
  ...
  let timer=0;function queue(){clearTimeout(timer);timer=setTimeout(push,700)}
  async function push(){
    if(G.pushing||!G.token)return;let state=mergeSharedIdentity(parse(read()));if(!state)return;const raw=JSON.stringify(state);if(raw===G.last)return;write(state);G.pushing=true;G.notice('Guardando…');const previousIdentity=G.identity;
    try{
      const a=await api('PUT',{version:G.ver,state});
      if(a.r.status===409&&a.x.server){try{localStorage.setItem('weddly_guests_conflict_backup',raw)}catch{}if(a.x.server.state){const latest=mergeSharedIdentity(a.x.server.state);write(latest);G.last=JSON.stringify(latest);G.identity=identityOf(latest)}G.ver=+a.x.server.version||G.ver;G.remote=true;G.f.src=G.APP+'&remote='+Date.now();G.notice('Actualizado desde el otro dispositivo');setTimeout(G.clearNotice,2500)}
      else if(!a.r.ok||!a.x.ok)throw 0;
      else{G.ver=+a.x.version||G.ver+1;G.last=raw;G.identity=identityOf(state);G.clearNotice();if(previousIdentity&&G.identity!==previousIdentity)G.f.src=G.APP+'&identity='+Date.now()}
    }catch{G.notice('Pendiente de conexión');setTimeout(()=>{G.pushing=false;queue()},2500);return}
    G.pushing=false
  }
  async function poll(){
    if(!G.token||G.pushing)return;const raw=read();if(raw!==G.last){queue();return}
    try{const a=await api('GET');if(!a.r.ok||!a.x.ok)return;const v=+a.x.version||0;if(v>G.ver&&a.x.state){let state=mergeSharedIdentity(a.x.state);const merged=JSON.stringify(state);if(merged!==JSON.stringify(a.x.state)){const p=await api('PUT',{version:v,state});if(p.r.ok&&p.x.ok)G.ver=+p.x.version||v+1;else G.ver=v}else G.ver=v;write(state);G.last=merged;G.identity=identityOf(state);G.remote=true;G.f.src=G.APP+'&remote='+Date.now();G.notice('Actualizado con los cambios de vuestra pareja');setTimeout(G.clearNotice,2500)}}catch{}
  }
  setInterval(()=>{const raw=read();if(G.remote){G.remote=false;G.last=raw;return}if(raw!==G.last)queue()},500);
  setInterval(poll,12000);addEventListener('online',()=>{queue();poll()});start()
})();
