(()=>{
  ...
  const RSVP_URL='guests-rsvp-operations-v2.html?v=140-mobile-ux';
  ...
  function setTopHash(id){if(SUITE){try{window.parent.postMessage({type:'wsd-suite-section',section:id},location.origin)}catch{}return}try{const u=new URL(window.top.location.href);u.hash=id;window.top.history.replaceState(null,'',u.href)}catch{}}
  ...
  function settings(d){d.getElementById('wsdAppSettings')?.remove();const b=d.getElementById('editWedding');if(!b)return;if(SUITE){b.style.setProperty('display','none','important');return}b.textContent='Ajustes';b.setAttribute('aria-label','Ajustes de la app');b.style.textDecoration='none';b.style.border='1px solid #d7d0c6';b.style.background='#fff';b.style.borderRadius='999px';b.style.padding='7px 11px'}
  ...
  function hook(d){if(!d?.documentElement||d.documentElement.dataset.wsdV129Prod==='1')return;d.documentElement.dataset.wsdV129Prod='1';d.addEventListener('click',e=>{const t=e.target?.closest?.('button,a');if(!t)return;const label=(t.textContent||'').trim();if(t.dataset?.go){setTimeout(()=>setTopHash(t.dataset.go),0);return}if(t.id==='editWedding'||label==='Ajustes'){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();if(!suiteOpen('settings'))topGo(SETTINGS_URL);return}if(t.id==='rsvpBtn'||/^Gestionar RSVP$/i.test(label)||/^Envíos y respuestas$/i.test(label)){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation?.();if(!suiteOpen('guests-rsvp',RSVP_URL))topGo(RSVP_URL)}},true)}
  function applyInitialHash(d){if(SUITE||hashApplied)return;hashApplied=true;const id=(window.top.location.hash||'').slice(1);if(!['hoy','invitados','mesas','listados'].includes(id))return;const b=d.querySelector(`#nav button[data-go="${CSS.escape(id)}"]`);if(b&&!b.classList.contains('on'))b.click()}
